...
export default function SavingsGoals() {
  ...
  useEffect(() => {
    const id = setTimeout(() => {
      ...
      goals.forEach((goal) => {
        newFormattedDates[goal.name] = new Date(
          goal.deadline,
  ...
  return (
    <Card className="bg-background max-h-120 gap-0 overflow-hidden rounded-lg p-0 shadow-none">
      ...
      <CardContent className="bg-background h-full space-y-3 overflow-auto p-3">
        {goals.map((goal) => {
          ...
          return (
            <div
              key={goal.name}
              className="space-y-3 rounded-md border bg-linear-to-tl from-secondary/30 p-5"
            >
