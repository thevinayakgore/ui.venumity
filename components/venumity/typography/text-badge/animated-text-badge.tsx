...
export default function TextBadgeAnimated() {
  const [selectedBadge, setSelectedBadge] = useState(0);
  const [speed, setSpeed] = useState(1);
  const [showAll, setShowAll] = useState(false);

  const renderAnimatedBadge = (badge: AnimatedBadge) => {
    const baseClasses = `inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium ${badge.color}`;
              ...
              {showAll ? (
                ...
                <div className="flex flex-col items-center gap-6">
                  ...
                  <p className="text-gray-600 dark:text-gray-400 text-center">
                    This badge has a &quot;
                    {animatedBadges[selectedBadge].animation}&ldquo; animation
                    effect
                  </p>
                </div>
              )}
