...
export default function LuxuryStack() {
            ...
            <div className="flex items-center justify-between mb-12">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-gray-400">Exclusive premium features for elite users</p>
                </div>
              </div>
                  ...
                  <div className="absolute inset-0 bg-linear-to-br opacity-0 group-hover:opacity-20 transition-opacity duration-300 blur-xl rounded-2xl"
                    style={{ 
                      background: `linear-gradient(to bottom right, ${
                        item.gradient?.split(' ')[1]?.replace('from-', '#') || '#ffffff'
                      }, ${
                        item.gradient?.split(' ')[3]?.replace('to-', '#') || '#000000'
                      })`
                    }}
                  />
                  ...
                  <div className="relative bg-linear-to-br from-gray-900/80 to-black/80 backdrop-blur-sm rounded-2xl p-8 border border-gray-800 group-hover:border-gray-700 transition-all">
                    <div className="flex items-start justify-between mb-6">
                      <div className="flex items-start gap-4">
                        <div className={`p-4 rounded-xl bg-linear-to-br ${item.gradient} shadow-2xl`}>
                          {item.icon}
                        </div>
                        <div>
                          <div className="flex items-center gap-3 mb-2">
                            <h3 className="text-2xl font-bold text-white">{item.title}</h3>
                            <span className={`text-sm font-medium px-3 py-1 rounded-full ${item.accentColor} bg-opacity-20`}>
                              {item.subtitle}
                            </span>
                          </div>
                    ...
                    <div className="flex items-center justify-between pt-6 border-t border-gray-800">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-gray-300 text-sm">VIP Feature</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <div className="flex -space-x-2">
                          {[1, 2, 3].map((i) => (
                            <div key={i} className="w-8 h-8 rounded-full bg-linear-to-r from-blue-500 to-purple-500 border-2 border-gray-900" />
                          ))}
                        </div>
                        <span className="text-gray-400 text-sm">+{item.id * 12} users</span>
                      </div>
                    </div>
                  </div>
                </motion.div>
