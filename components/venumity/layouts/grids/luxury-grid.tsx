...
import { Crown, Gem, Sparkles, Target, Zap, Users, BarChart3, Award, ChevronRight } from "lucide-react";
              ...
              <div className="flex items-center gap-4 mb-6 lg:mb-0">
                <div className="relative">
                  <motion.div
                    ...
                    className="w-24 h-24 rounded-full bg-linear-to-r from-yellow-400 via-orange-400 to-yellow-600 flex items-center justify-center shadow-2xl shadow-yellow-500/30"
                  >
                    <Crown className="w-12 h-12 text-white" />
                  </motion.div>
                  <Sparkles className="absolute -top-2 -right-2 w-10 h-10 text-yellow-300" />
                </div>
                <div>
                  <h2 className="text-4xl font-bold bg-linear-to-r from-yellow-300 via-orange-300 to-yellow-300 bg-clip-text text-transparent">
                    Luxury Executive Grid
                  </h2>
                  <p className="text-gray-400 mt-2">Exclusive metrics and premium insights for VIP members</p>
                </div>
              </div>
              
              <div className="flex items-center gap-4">
                <Award className="w-10 h-10 text-yellow-400" />
                <span className="text-yellow-300 text-xl font-semibold">VIP Executive</span>
              </div>
            </div>
                ...
                >
                  ...
                  <div className="relative bg-linear-to-br from-gray-900/80 to-black/80 backdrop-blur-sm rounded-2xl p-8 border border-gray-800 group-hover:border-gray-700 transition-all h-full">
                    <div className="flex items-start justify-between mb-6">
                      <div className={`p-5 rounded-xl bg-linear-to-br ${metric.gradient} shadow-2xl`}>
                        {metric.icon}
                      </div>
                      <motion.div
                        animate={{ x: [0, 5, 0] }}
                    ...
                    <div className="mb-4">
                      ...
                      <p className={`text-xl font-semibold ${metric.accentColor}`}>{metric.title}</p>
                      <p className="text-gray-400 mt-1">{metric.subtitle}</p>
                    </div>
                    
                    <div className="h-1 w-0 group-hover:w-full bg-linear-to-r opacity-50 rounded-full transition-all duration-500 mt-6"
                      style={{
                        background: (() => {
                          const g = metric.gradient ?? "";
                          const parts = g.split(" ");
