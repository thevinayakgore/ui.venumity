...
export const BottomFooter = () => {
  ...
  return (
    <div
      className={cn(
        "relative p-5 text-xs font-semibold tracking-wide! border-t overflow-hidden w-full",
        pathname?.startsWith("/components") ||
          pathname?.startsWith("/docs") ||
          pathname?.startsWith("/resources") ||
          pathname?.startsWith("/changelog")
          ? "bg-foreground/3"
          : "border-dashed mt-5 md:py-8",
      )}
    >
      {/* Bottom line */}
      <div className="flex items-center justify-between leading-none text-foreground/40 w-full">
        ...
        {SOCIAL_LINKS.map((item, index) => (
          <Tooltip key={index}>
            <TooltipTrigger
              asChild
              className={cn(
                "w-auto hover:-translate-y-6 hover:rotate-0 z-0 hover:z-20 transition-all duration-400 overflow-hidden",
                item.rotate,
                pathname?.startsWith("/components") ||
                  pathname?.startsWith("/resources") ||
                  pathname?.startsWith("/changelog")
                  ? "h-10 mb-1 rounded-sm"
                  : "h-12 rounded-md",
              )}
            >
...
export default function Footer() {
  ...
  return (
    <footer className="p-5 md:p-10 m-auto max-w-400 w-full">
      <div className="bg-foreground/5 p-2 pb-0! border-b-30 rounded-[1.3rem] rounded-b-xl overflow-hidden">
        {/* 3D Morquee Banner */}
        <section className="relative shadow-2xl/10 bg-foreground/5 backdrop-blur-md rounded-2xl overflow-hidden w-full min-h-150">
          <ThreeDMarquee
            className="pointer-events-none absolute inset-0 bg-background! h-full w-full"
            images={images}
          />
          <div className="absolute inset-0 z-20 flex flex-col items-center m-auto gap-5 p-5 md:p-10 bg-white/5 backdrop-blur-3xl text-white shadow-2xl rounded-4xl max-w-270 w-full h-fit">
            <div className="flex items-start justify-between gap-5 md:gap-10 m-auto w-full">
              <div className="flex items-center gap-5 md:gap-7 w-ull">
                ...
                <h2 className="text-2xl md:text-[2.4rem] font-semibold tracking-tight leading-none">
                  <span className="bg-clip-text text-transparent bg-linear-to-br from-white via-white to-white/30">
            ...
            <p className="text-base tracking-wide opacity-70 w-full">
        ...
        <section className="relative mt-3 w-full">
          ...
          <div className="p-5 grid grid-cols-7 items-start justify-between gap-5 md:gap-10 w-full">
            <div className="w-xs col-span-2 space-y-3">
              <div className="flex items-center gap-3">
                <Image
                  ...
                  width={500}
                  height={500}
                  className="border-2 border-white shadow-lg shadow-primary/40 rounded-md w-max h-10"
                />
              ...
              <div className="flex flex-wrap items-center gap-2.5 mt-5 list-none">
                <FooterLink
                  href="https://pro.venumity.com/legal/license"
                  className="flex items-center gap-2 pl-2.5 pr-3! h-7 text-xs! leading-none bg-foreground/5! text-foreground/60! border border-foreground/15 hover:bg-primary/15! hover:text-primary! hover:border-primary/40 rounded-full"
                >
            ...
            <div className="col-span-2 flex flex-col gap-3 justify-center mx-auto w-fit">
            ...
            {/* <div className="flex flex-col gap-3 justify-center mx-auto w-fit">
              ...
              <ul className="space-y-1.5">
                ...
                <FooterLink href="https://pro.venumity.com" external>
                  pro.venumity
                </FooterLink>
                <FooterLink href="https://blog.venumity.com" external>
                  blog.venumity
                </FooterLink>
                <FooterLink href="https://www.venumity.com" external>
                  www.venumity
                </FooterLink>
              </ul>
            </div> */}
    ...
    return (
      <li>
        <Link
          ...
          rel="noreferrer"
          className={cn(
            "flex items-center gap-1.5 text-xs md:text-sm text-foreground/50 hover:text-foreground tracking-wide w-fit",
            className,
          )}
        >
          {children}
