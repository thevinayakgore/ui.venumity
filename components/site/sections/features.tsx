...
function ComponentsShowcase() {
  ...
  return (
    <div className="relative mt-5 w-full">
      <Link href={`/components${currentCard.path}`}>
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="relative cursor-pointer p-2 group bg-card shadow-2xl shadow-blue-500/30 rounded-3xl overflow-hidden transition-all duration-500 w-full"
          >
            <div className="aspect-square relative shadow-xl/15 max-h-80 w-full overflow-hidden rounded-2xl transition-all duration-500">
            ...
            <div className="p-4 pt-5">
              <div className="flex items-start justify-between font-semibold">
                <h3 className="text-lg whitespace-nowrap truncate">
                  {currentCard.title}
                </h3>
                <span className="text-sm font-semibold px-2 py-1 rounded-sm bg-primary/20 text-primary">
              ...
              <p className="text-sm font-semibold tracking-wide line-clamp-2 opacity-40 mt-2 mb-4">
              ...
              <div className="flex flex-wrap gap-1.5">
                {currentCard.tags.slice(0, 2).map((tag, idx) => (
                  <span
                    key={`${tag}-${idx}`}
                    className="text-xs px-2.75 h-6.5 flex items-center bg-foreground/5 border text-foreground/60 capitalize font-medium trackism-wide rounded-sm"
                  >
                    {tag}
                  </span>
                ))}
                {currentCard.tags.length > 2 && (
                  <span className="text-xs px-2.75 h-6.5 flex items-center bg-foreground/5 border text-foreground/60 font-medium rounded-sm">
                    +{currentCard.tags.length - 2}
                  </span>
                )}
              </div>
      ...
      {/* Card indicators */}
      <div className="flex items-center justify-center gap-1.5 mt-5 md:mt-10">
          ...
          <button
            ...
            className={`h-3 rounded-full transition-all duration-500 ${
              index === activeCardIndex
                ? "w-10 bg-blue-600"
                : "w-3 bg-blue-600/40 backdrop-blur-md hover:bg-blue-600"
            }`}
...
function FreeOpenSourceDemo() {
  return (
    <div className="absolute -top-20 -left-15 black-ops-one z-0 text-[30rem] tracking-tighter text-transparent bg-clip-text bg-linear-to-br from-pink-500 via-pink-500/15 opacity-20 leading-none">
      MIT
    </div>
  );
}
      ...
      <div className="relative p-5 bg-black rounded-t-4xl w-full h-full">
        ...
        <div className="flex flex-col items-start gap-2 z-50 transform-gpu group-hover/dummyCard:-translate-y-30 transition-all duration-500 mt-4 p-3 bg-white/10 backdrop-blur-lg text-white shadow-xl rounded-2xl w-full">
          <Image
            ...
            width={500}
            height={500}
            className="aspect-video object-cover shadow-lg/20 rounded-xl w-full"
          />
          <div className="space-y-2 p-2 w-full">
            <h3 className="text-lg tracking-wide leading-none">
              Venumity UI Hero
            </h3>
            <p className="text-xs font-semibold tracking-wide opacity-50">
...
function CustomizationInfo() {
  return (
    <div className="relative space-y-5 p-5 md:p-8 bg-foreground/3 backdrop-blur-lg rounded-2xl w-full">
      {/* Core message */}
      ...
      {/* Key selling point */}
      <div className="space-y-1 text-sm text-foreground/60 w-full">
...
function SmoothAnimationDemo() {
  ...
  return (
    <div className="absolute inset-0 top-0 left-1/5 z-60 bg-background rounded-b-3xl overflow-hidden h-fit w-120">
      <div className="absolute inset-0 bg-linear-to-r from-background z-50 rounded-bl-3xl h-23 w-1/3" />
      <div className="absolute top-0 right-0 bg-linear-to-l from-background z-50 rounded-br-3xl h-23 w-1/3" />
      ...
      <div className="absolute -top-5 left-0 z-40 opacity-40 bg-linear-to-l from-transparent via-indigo-500 to-transparent blur-xl rounded-full h-10 w-full" />
      ...
      <div className="absolute -bottom-5 left-0 z-40 opacity-40 bg-linear-to-l from-transparent via-indigo-500 to-transparent blur-xl rounded-full h-10 w-full" />
    </div>
  );
}
...
export default function Features() {
  ...
  return (
    <section className="bg-foreground/5 backdrop-blur-sm w-full">
      <div className="flex flex-col gap-5 md:gap-10 lg:gap-20 p-5 md:p-10 lg:py-20 m-auto max-w-400 w-full">
        <div className="flex flex-col items-center justify-center m-auto text-center max-w-fit">
          <h2
            className={cn(
        ...
        <motion.div
          ...
          className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 lg:grid-rows-3 gap-6 w-full"
        >
          {features.map((feature, index) => {
            ...
            return (
              ...
              >
                <Card
                  className={`relative p-5! md:p-10! h-full transition-all bg-linear-to-br! ${feature.bg} inset-shadow-sm inset-shadow-foreground/5 ring-0! shadow-none! rounded-2xl overflow-hidden`}
                >
                  ...
                  <CardHeader className="p-0! z-30!">
                    <span className={`size-15 ${feature.iconColor} mb-5`}>
                      {feature.icon}
                    </span>
                    <CardTitle
                      className={`text-4xl md:text-[2.5rem] font-semibold tracking-tight text-transparent bg-clip-text bg-linear-to-br ${feature.textColor} pb-3 w-fit`}
                    >
                      {feature.title}
                    </CardTitle>
                    <CardDescription className="text-lg text-foreground/50 tracking-wide leading-6 max-w-md">
                      {feature.description}
                    </CardDescription>
                  </CardHeader>
                  ...
                  {feature.title !== "Free & Open Source" &&
                    feature.title !== "Smooth Animations" && (
                      <CardContent className="p-0! z-50 transform-gpu text-base text-foreground! leading-7 tracking-wide max-w-2xl">
                        {feature.content}
                      </CardContent>
                    )}
                  ...
                  <motion.svg
                    ...
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    className={`absolute -bottom-35 -right-25 -rotate-10 z-0 size-100 ${feature.iconColor} opacity-20`}
                  >
